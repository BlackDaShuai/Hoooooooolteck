...
{
	CKCU_PeripClockConfig_TypeDef dht11Clock = {{ 0 }};
	dht11Clock.Bit.PB = 1; //使能PC，//再将所需要的时钟以访问成员变量的形式，其中bit是联合体内 含3*32位的结构体 ，其下成员均为对应外设的时钟位，置一即该时钟的配置寄存器写入一
	CKCU_PeripClockConfig(dht11Clock, ENABLE); //一定要先使能时钟，再配置下面的
	// GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_8, GPIO_DIR_OUT); //设置PC1为输出

    DHT11_Rst();
	// DHT11_Check();
    printf("check DHT11(0=success):%d\r\n",DHT11_Check());
