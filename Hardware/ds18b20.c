...
//  	GPIO_Init ( DS18B20_DQ_GPIO_PORT , &GPIO_InitStructure );
	...
	CKCU_PeripClockConfig_TypeDef dsClock = {{ 0 }};
	dsClock.Bit.PB = 1; //使能PB，//再将所需要的时钟以访问成员变量的形式，其中bit是联合体内 含3*32位的结构体 ，其下成员均为对应外设的时钟位，置一即该时钟的配置寄存器写入一
	CKCU_PeripClockConfig(dsClock, ENABLE); //一定要先使能时钟，再配置下面的
